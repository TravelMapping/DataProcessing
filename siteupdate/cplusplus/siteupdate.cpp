...
{	ifstream file;
	...
	{	std::cout << et.et() << "Start writing database file " << Args::databasename << ".sql.\n" << std::flush;
		sqlthread=thread(sqlfile1, &et, &all_regions, &continents, &countries, &clin_db_val, &updates, &systemupdates, &term_mtx);
	}
      #endif
	...
	if   (Args::errorcheck)
		...
		sqlfile1(&et, &all_regions, &continents, &countries, &clin_db_val, &updates, &systemupdates, &term_mtx);
	      #endif
