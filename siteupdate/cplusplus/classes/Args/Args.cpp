...
/* D */ std::string Args::databasepath = "."; // <-- new variable with default "."
...
{	const char* slash = strrchr (argv[0], '/');
    ...
    {	     if ARG(0, "-e", "--errorcheck")		 errorcheck = 1;
        ...
        else if ARG(1, "-D", "--databasepath")		{databasepath     = argv[++n];} // <-- new argument
...
{	std::string indent(strlen(exec), ' ');
    ...
    std::cout  <<  indent << "        [-u USERLISTFILEPATH] [-x USERLISTEXT] [-d DATABASENAME] [-D DATABASEPATH] [-l LOGFILEPATH]\n";
    ...
    std::cout  <<  "  -D DATABASEPATH, --databasepath DATABASEPATH\n";
    std::cout  <<  "		        Path to write database .csv files\n";
